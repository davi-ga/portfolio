import { Mail, MapPin, FileText } from "lucide-react";
import { C, F } from "../theme";
import SectionWrapper from "../components/SectionWrapper";
import CTAButton from "../components/CTAButton";
import IconLink from "../components/IconLink";
import { HERO } from "../data/content";
import { useIsMobile } from "../hooks/useIsMobile";

const Github = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={props.size ?? 24} height={props.size ?? 24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

const Linkedin = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={props.size ?? 24} height={props.size ?? 24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect width="4" height="12" x="2" y="9" />
    <circle cx="4" cy="4" r="2" />
  </svg>
);

/**
 * HeroSection — full-width opening section.
 * Desktop: asymmetric two-column grid.
 * Mobile: single column stack.
 */
const HeroSection = () => {
  const isMobile = useIsMobile();

  return (
    <SectionWrapper
      id="hero"
      divider={false}
      style={{
        paddingTop: isMobile ? "100px" : "176px",
        paddingBottom: isMobile ? "48px" : "72px",
      }}
    >
      <div
        style={{
          display: "grid",
          gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr",
          gap: isMobile ? "40px" : "0",
        }}
      >
        {/* ── Name block ── */}
        <div>
          <p
            style={{
              fontFamily: F.mono,
              fontSize: "10px",
              letterSpacing: "0.12em",
              color: C.muted,
              marginBottom: "28px",
              display: "flex",
              alignItems: "center",
              gap: "6px",
            }}
          >
            <MapPin size={11} strokeWidth={1.5} color={C.muted} />
            {HERO.location}
          </p>

          <h1
            style={{
              fontSize: isMobile ? "clamp(40px, 12vw, 60px)" : "clamp(44px, 5vw, 72px)",
              fontWeight: 700,
              lineHeight: 1.0,
              letterSpacing: "-0.03em",
              color: C.text,
              margin: 0,
              fontFamily: F.sans,
              textAlign: "left",
            }}
          >
            <span style={{ color: C.accent }}>{HERO.name.split(" ")[0]}</span>
            <br />
            <span>
              <span style={{ color: C.accent }}>Ga</span>
              <span style={{ color: C.text }}>ldino</span>
            </span>
          </h1>

          <div style={{ marginTop: "22px", width: "36px", height: "1px", background: C.accent }} />

          <p
            style={{
              marginTop: "22px",
              fontFamily: F.mono,
              fontSize: "12px",
              color: C.muted,
              letterSpacing: "0.04em",
              lineHeight: 1.65,
              textAlign: "left",
            }}
          >
            {HERO.headline.join("\n").split("\n").map((line, i) => (
              <span key={i}>{line}{i === 0 && <br />}</span>
            ))}
          </p>
        </div>

        {/* ── Bio + CTA ── */}
        <div
          style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: isMobile ? "flex-start" : "flex-end",
            paddingLeft: isMobile ? "0" : "64px",
            textAlign: "left",
          }}
        >
          <p
            style={{
              fontSize: "15px",
              color: C.muted,
              lineHeight: 1.85,
              maxWidth: isMobile ? "100%" : "380px",
              margin: "0 0 36px",
              fontFamily: F.sans,
              fontWeight: 300,
            }}
          >
            {HERO.bio}
          </p>

          <div style={{ display: "flex", gap: "16px", alignItems: "center", flexWrap: "wrap" }}>
            <CTAButton href={HERO.contact}>CONTATO</CTAButton>

            <div style={{ display: "flex", gap: "18px" }}>
              <IconLink href={HERO.github} Icon={Github} label="GitHub" target="_blank" rel="noopener noreferrer" />
              <IconLink href={HERO.linkedin} Icon={Linkedin} label="LinkedIn" target="_blank" rel="noopener noreferrer" />
              <IconLink href={`mailto:${HERO.email}`} Icon={Mail} label="Email" />
              <IconLink href="/curriculo.pdf" Icon={FileText} label="Currículo" target="_blank" rel="noopener noreferrer" />
            </div>
          </div>
        </div>
      </div>

      {/* ── Bottom rule: discipline tags ── */}
      <div
        style={{
          marginTop: isMobile ? "40px" : "72px",
          borderTop: `0.5px solid ${C.border}`,
          display: "flex",
          justifyContent: "space-between",
          paddingTop: "20px",
          flexWrap: "wrap",
          gap: "8px",
        }}
      >
        {HERO.tags.map((tag) => (
          <span
            key={tag}
            style={{
              fontFamily: F.mono,
              fontSize: "10px",
              letterSpacing: "0.1em",
              color: C.border,
              textTransform: "uppercase",
            }}
          >
            {tag}
          </span>
        ))}
      </div>
    </SectionWrapper>
  );
};

export default HeroSection;
